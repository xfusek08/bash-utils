...
#
# Usage:
#     install_deb_package_url <url> [-h|--help]
#
# Options:
...
#
# Examples:
#     install_deb_package_url https://example.com/package.deb
#
# Notes:
...
unalias install_deb_package_url 2>/dev/null # Remove potential alias

install_deb_package_url() {
    # Parse options and arguments
    local help=false
    local opts
    opts=$(getopt -o h --long help -n 'install_deb_package_url' -- "$@")
    if [ $? -ne 0 ]; then
    ...
    if [ "$help" == "true" ]; then
        cat <<EOF
Usage: install_deb_package_url <url> [OPTIONS]
...
EOF
    ...
    # Install the package using install_deb_package alias
